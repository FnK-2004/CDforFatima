...
int Fcount = 0;
int i, k;
int searchG(const char* lexeme){      
    for(i=0; i<Gcount; i++){          
        if(globalTable[i].lexeme==lexeme){    
            return 1;}
    }
    return 0;                        
}
...
int searchL(const char* lexeme, int funcIndex){
    if(funcIndex>=0 && funcIndex<Fcount){
        int a=functionTable[funcIndex].ptr_LTable;         
...
void insertGlobalSymbol(const char* lexeme, const char* type, int size) {
    if (searchG(lexeme) == 0) {
        ...
        globalTable[Gcount].ptrToSymb = global_id;
        Gcount++;
        global_id++;
    }
}
...
void displayglobalTable() {
    ...
    for (int i = 0; i < Gcount; i++) {
        cout<<"   "<<i + 1 <<"\t" << globalTable[i].lexeme << "\t\t Function\t\t" << globalTable[i].ptrToSymb << endl;
    }
}
...
struct TokenByFatima getNextToken(FILE *fptr) {
    int i=0, j, flag;
    char c, c1, str[100];
...
int main() {
    char filename[100];
    cout<<"Enter the filename to open for reading: ";
    cin>>filename;
    ...
    while(true) {
        ...
        if(token.type==NULL) {
            ...
            int currentFunctionIndex = Fcount - 1; 
